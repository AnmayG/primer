...
<!-- DK: Give them a tour of hidden files in the Files tab. DONE - In Creating R Projects -->
...
<!-- AG: There aren't any subheadings in this section, so I'm not including them for now. It becomes pretty long without them though. -->
...
R Markdown files, or .Rmd files, are a type of file that can generate *reports* from R code. Basically, you can write both the graph and the explanation about the graph within the same file. You can then *knit* the file in order to render the text and make it all look nice.

Let's walk through creating an R Markdown file.
