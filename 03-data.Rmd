...
CSV files (and their counterparts) are easily transferable between computers and programming languages and are extremely simple because they're effectively just text files that have a special format. Additionally, they're very easy to parse through small amounts of data due to th eir simple nature. 
However, due to their simple nature, CSV files are only able to move basic data and only as text values. They also have poor support for special characters like commas, which can make the dataset harder to organize and understand by adding a new column for a few specific entries.
...
You can often find CSV files on websites like [kaggle](https://kaggle.com) as well as by exporting from an Excel spreadsheet.
...
#### Using SQL code
<!-- AG: I'm devoting a paragraph to SQL and what it is. I'm doing this because I don't want people to misunderstand that you can just use dbplyr for everything, as well as that dbplyr is actually doing the SQL code for you. -->

<!-- AG: The caption seemed to knit ok, but I'm not sure if I should use it. -->
...
<!-- AG: Is it OK to write "in this course" or should I put something else? -->
...
-- Instead, you need to tret all of these lines of code as a single statement
...
SQL is more versatile that `dbplyr`, but it's also a lot more confusing because it's not as straightforward. This creates one golden rule:
