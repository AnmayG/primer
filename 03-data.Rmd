...
<!-- Explain that parsing failure is designed to be a failure. -->
...
<!-- Include a section about how you're supposed to put \ whenever there's a space, since 90% of people will have a space in their file name. However, because we don't have a section about it, we get a lot of confusion about why somebody's thing isn't working. -->
...
# weird. It's not necessary on MacOS and cause an error as well.
...
# no real benefit. It still just acts as a ggplot and restricts the user to only
# using ggplot functions. It's more practical to save the data set itself as
# the people you're sending it to can then generate the graph from the data.
# We're using a graph here to better show how the rds file can be used to change
...
It's important to remember that other software and programming languages cannot read rds files naturally, so you should only share rds files with other people who use R.
...
<!-- Why is all of this code commented out? This passage is incomprehensible without it. Because this whole section sucks! Get this to work. -->
...
<!-- AG: I realize that there's probably been a lot of work put into this, but if we're not allowed to web scrape off Craigslist why bother using it as an example? Just use Wikipedia as the example and show how the TOS differ. -->
