...
###Doing operations on SQLite Databases
...
library(DBI)
library(RSQLite)
# This is commented out because it just causes a 404 error since this technically doesn't exist yet
# download.file(url = "https://github.com/PPBDS/primer/blob/master/03-data/data/employee_data.db?raw=true",
#               dest = "employee_data.db", mode = "wb")

#change this to just employee_data.db
con <- dbConnect(RSQLite::SQLite(), "03-data/data/employee_data.db")
```
...
dbListTables(con)
```
We can use this list of table names and access each table one by one.

These are randomly generated employee reviews and names, so they are very nonsensical.

<!-- AG: This looks pretty long once it's knit, as there's like 3 tables in a row. -->
<!-- AG: Since the only sample databases I found were copyrighted and stuff like that, I decided that it was easier to just make my own SQLite database using SQLite Studio and programs like that. I then used random generators like https://randommer.io/Text/Review to copy and paste in data or just used the "populate" function to randomly generate data. -->

```{r}
# The `kable()` functions are just to make the table look nice and are not strictly necessary
t1 <- head(tbl(con, "person"))
kable(t1, caption = "\"Person\" Table")
t2 <- head(tbl(con, "ratings"))
kable(t2, caption = "\"Ratings\" Table")
t3 <- head(tbl(con, "tasks"))
kable(t3, caption = "\"Tasks\" Table")
```
 
As you can see, each table has a common column: the ID column. However, they all have different information linked to that ID, such as "person" having the names of employees and "ratings" having a review for the employee.
