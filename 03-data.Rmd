...
We can do the same thing with classes that we did with IDs, using `html_elements()` to sort through them. You use a `.` instead of a `#` to show that it's a class. In order to sort by multiple classes, just combine the statements together.
...
These chains are known as **CSS Selectors** and they're an important part of data science because they allow us to find data and information without having to download every file or copying everything down. Instead, we can just use the website's inbuilt code to get the information.
