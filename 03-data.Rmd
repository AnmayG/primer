...
<!-- Rewrite entire webscraping section. Delete everything you don't like. Look closely at the current version of the tutorial. Rewrite this section so that it will be easy to fold the tutorial questions into it at some point in the future. DONE -->

<!-- Start of chapter, mention that all the files we are working on live in this directory, called `file_dir <- "https://raw.githubusercontent.com/PPBDS/primer.tutorials/master/inst/www/"`. Then, define `file_1 <- paste0(file_dir, "test_1.csv")` or whatever. DONE -->

<!-- The tutorial includes a lot of arguments like na and comment, but that makes the book feel like it's really long once it's actually put into text form with paragraphs and stuff. True! So, what do we include and what do we not? I I don't know, I don't really have any feelings either way. It makes sense to have it in the book though since people will likely refer to the book rather than the documentation when they have problems. DONE -->
...
<!-- AG: I fixed the error in TODO.txt, the problem was that the CSV files don't have an ending line. This can be avoided by doing `warn = FALSE` -->
...
Take `test_7.csv`.
```{r}
file_3 <- paste0(github_url, "test_7.csv")
...
```{r}
file_4 <- paste0(github_url, "test_3.csv")
...
# different like "\\t" This varies for every symbol, but you can find most delim
...
<!-- AG: This is my first time doing SQL code and I'm more experienced with Java/Python, so I formatted it like how Java and Python are formatted with the parentheses acting as brackets instead. This may be heresy in the SQL world, so please tell me if I have to fix it. I did some research and found an online SQL formatter here: https://sqlformat.org/ I'm just going to use this syntax because there are a lot of conflicting things and this is the one .org website I found.-->
...
<!-- AG: I noticed that David Kane is never actually listed anywhere in the book, is it ok to have your name in a meme like the one below? -->
...
<!-- AG: I added some CSS rules so that I can keep HTML in a box instead of leaving it out in the open. This knitted on my computer, but please tell me if it causes problems in the final build. -->
...
<!-- AG: I'm pretty sure I can link to the rest of the book like what I did here. Just making sure since I'm not really sure about how to style a link to the rest of the book. -->

Now, we're doing data science. And let's say you find some really cool website off of the internet that has a lot of really useful data, but there's no download file ([and it's legal to scrape off of it](https://ppbds.github.io/primer/data.html#are-you-allowed-to-scrape-that-website)). That means that you have to pull the information from the HTML code.

We can do this by using the **`rvest`** package like I talked about earlier. This is essentially a package that allows you to scrape information from HTML code, allowing you to scrape the information off of the website overall.
...
First off, we need to use the `minimal_html()` function so that we can get an R object that we can then work on. Think about it like changing the information from HTML code into something that R can understand.

```{r}
# You can't use HTML code that's structured properly in R, so we condense it 
# into one line, like what's shown below.

raw_html_1 <- "<html><p>Hello World</p></html>"
raw_html_1 %>%
  minimal_html()
```
...
<!-- AG: I'm wondering how closely I should stick to the tutorial problems. I know that we're going to be integrating them into each other soon, but the tutorial problems don't have as much information as I would like (such as the paragraph tags) and they're not as reusable (I can do both examples with one piece of code vs the two used in the tutorial). Right now, I'm going to use my own examples because I feel like it makes the section more understandable. Also, the tutorials shouldn't be too close to the primer since we're likely not going to be combining them too soon (I think) and I personally don't think that they should cover the exact same things with the exact same examples, especially for something as in-depth as HTML code. -->
...
<html>
  <table>
    <tr>
      ...
      <td>
        This is some other important info.
      </td>
    </tr>
    <tr>
      <td>
...
<!-- AG: This just feels wrong, but it doesn't run any other way. So if I want to keep the HTML code syntax without going miles off screen, I'm going to be doing this. The error that it gives me is: `Error: unexpected `<` in "<"`. It's weird, dumb, and stops me from using the proper syntax without having to circumvent it. -->

```{r}
# We're using a trick here where every line of HTML is pasted together, allowing
# us to keep the original syntax even though the line is super long.

raw_html_2 <- paste0("<html>",
                       "<table>",
                         "<tr>",
                           "<td>", 
                             "This is some important info.",
...
However, what if we want only a specific column or type of tag? That's what CSS selectors are for.

#### CSS selectors

**CSS**, or **C**ascading **S**tyle **S**heets, is a coding language that defines the style of webpages. You may have noticed earlier that the HTML code we output earlier was drab and boring. But websites aren't drab and boring, they're fun and exciting! They do this by using CSS to add special rules to specific elements.

Now, there are a number of ways to do this. The first one is by using an **ID** to tell the webpage that they want this specific element to be a specific style. We can see an example of this below, where we set the element with the ID `red-id` to be red.

<!-- AG: This is a version that's very different from that of the tutorial because the examples there aren't great for explaining CSS selectors. There's no explanation and you end up just following the tutorial without really understanding anything (at least that's what I felt like, and what my experience as a TF showed). As such, I'm using custom examples and I'm only going to use the tutorial explanation at the end. This may interfere with future work though. -->
...
We can then use that ID in the HTML code.

```{.html}
<html>
<p> This is not red :( </p>
<p> This is not red :( </p>
<p id = 'red-id'> Amazing! </p>
<p> This is not red :( </p>
<p> This is not red :( </p>
<p> This is not red :( </p>
<p> This is not red :( </p>
...
<p class = "artificial-paragraph"> This is not red :( </p>
<p class = "artificial-paragraph" id = 'red-id'> Amazing! </p>
<p class = "artificial-paragraph"> This is not red :( </p>
<p class = "artificial-paragraph"> This is not red :( </p>
<p class = "artificial-paragraph"> This is not red :( </p>
</html>
...
Now, there's a really clever trick that we can use here. Let's say that we only want that one piece of text that's colored red, but we don't know exactly where it is. We can actually use that CSS id to get that piece of text, using the same function `html_element()`. Just plug in the ID of the element that you want with a `#` in front to tell R that it's an ID.

```{r}
raw_html_3 <- paste0("<p> This is not red :( </p>",
                     "<p> This is not red :( </p>",
                     "<p id = 'red-id'> Amazing! </p>",
                     "<p> This is not red :( </p>",
                     "<p> This is not red :( </p>",
...
Now, we talked about using IDs for getting an element, but what about if we want multiple elements to be formatted the same way? We can just use classes!

CSS classes are a way for multiple elements to have the same style, rather than being limited to a unique ID. Elements can also have multiple classes that combine all of the styles together.
...
```{.html}
<html>
<p class = 'green-bg'> This is green :) </p>
<p class = 'red-class'> This is red :) </p>
<p class = 'red-class green-bg'> Amazing! </p>
<p class = 'red-class'> This is red :) </p>
<p> This is not red :( </p>
<p class = 'green-bg'> This is green :) </p>
<p> This is not red :( </p>
...
<div class = "artificial-html-div">
<html>
<p class = 'green-bg  artificial-paragraph'> This is green :) </p>
<p class = 'red-class  artificial-paragraph'> This is red :) </p>
<p class = 'red-class green-bg  artificial-paragraph'> Amazing! </p>
<p class = 'red-class  artificial-paragraph'> This is red :) </p>
<p class = 'artificial-paragraph'> This is not red :( </p>
<p class = 'green-bg  artificial-paragraph'> This is green :) </p>
<p class = 'artificial-paragraph'> This is not red :( </p>
...
We can do the same thing with classes that we did with IDs, using `html_elements()` to sort through them. You use a `.` instead of a `#` to show that it's a class. In order to sort by multiple classes, just combine the statements together. If you want to get all of the elements from multiple classes, use a comma to separate them.

```{r}
raw_html_4 <- paste0("<html>",
                     "<p class = 'green-bg'> This is green :) </p>",
                     "<p class = 'red-class'> This is red :) </p>",
                     "<p class = 'red-class green-bg'> Amazing! </p>",
                     "<p class = 'red-class'> This is red :) </p>",
...
raw_html_4 %>%
  minimal_html() %>%
  html_elements(".red-class") %>%
  html_text2()

raw_html_4 %>%
  minimal_html() %>%
  html_elements(".red-class.green-bg") %>%
  html_text2()
...
<!-- AG: I'm kind of conflicted here because there's a perfectly good video that shows exactly how we can use SelectorGadget on Wikipedia pages, but it doesn't line up with the tutorials. For now, I'm going to comment out the video for future reference and just follow along with the tutorial. -->
...
<!-- AG: This counts as noncommercial use right? OpenSecrets says it's open to noncommercial use, but we have to request permission for commercial use. -->
...
<!-- AG: I'm just providing the straight-up graph in this case so that the rest of the tutorial can just be copy-pasted into here. There's a lot of graph building exercises in the tutorial that honestly doesn't really fit in this chapter of the book, which is all about data. -->
